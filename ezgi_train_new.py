...
df = df[["comment_text"] + all_labels].dropna()
df[all_labels] = (df[all_labels] >= .5).astype(int)

X = df["comment_text"].values.reshape(-1,1)
Y = df[all_labels].values.astype(np.float32)
...
ds_tr_n, ds_va_n, ds_te_n = map(lambda d: tok_ds(d, normal_labels), (train,val,test))
ds_tr_i, ds_va_i, ds_te_i = map(lambda d: tok_ds(d, imbalanced_labels ), (train,val,test))
...
model_n = DualHeadBert(config, pos_weight_n)
model_i = DualHeadBert(config, pos_weight_n)
...
print("▶ Head‑N (threat / insult / obscene / id‑attack)");
tr_n.train()
tr_n.model.save_pretrained("saved_model_n")

print("▶ Head‑I (identity labels)");
tr_i.train()
